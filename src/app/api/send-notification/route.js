...
function buildReactivationEmailHtml({ distributorName, contactWhatsApp }) {
  return `
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
  ...
  <div style="max-width:560px;margin:0 auto;padding:24px;">
    <div style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.12);">
      ...
      <div style="background:#f8fafc;padding:20px 24px;text-align:center;border-top:1px solid #e2e8f0;">
        <p style="margin:0 0 4px;font-size:11px;color:#94a3b8;">Greenland Products S.A. de C.V.</p>
        <p style="margin:0;font-size:11px;color:#94a3b8;">www.greenland-products.com.mx</p>
      </div>
    </div>
  </div>
</body>
...
export async function POST(request) {
  try {
    ...
    if (type === 'new_order') {
      ...
      const { distributorName, distributorEmail: reactivationEmail, contactWhatsApp } = body;
      if (reactivationEmail) {
        emails.push({
          from: 'Greenland Products <ventas@greenland-products.com.mx>',
          to: [reactivationEmail],
          subject: `${distributorName}, tu cuenta de distribuidor está esperándote 🌿`,
          html: buildReactivationEmailHtml({
            distributorName,
            contactWhatsApp: contactWhatsApp || 'wa.me/528341234567',
          }),
        });
      }
    }
